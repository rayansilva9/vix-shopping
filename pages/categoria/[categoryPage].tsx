  ...
  return (
    <>
      <div className="min-h-screen min-w-screen flex bg-zinc-100 overflow-visible relative pt-8 md:px-6 xl:px-10 ">
        <div style={{ boxShadow: '#0000003f 0px 3px 10px -5px', position: 'sticky', top: '10px', zIndex: 999, }}
          ...
          <div className="flex flex-col items-start gap-3 my-4 w-full px-4">
            <div className="flex justify-between w-full">
              ...
              <div className="inline-flex gap-2 items-center justify-end  text-gray-400">
                <IoApps onClick={() => { setModeView("grid") }} className={`text-xl cursor-pointer ${modeView == 'grid' ? 'text-black' : 'text-gray-400'}`} />
                <MdOutlineFormatListBulleted onClick={() => { setModeView("list") }} className={`text-2xl cursor-pointer ${modeView == 'list' ? 'text-black' : 'text-gray-400'}`} />
              </div>
        ...
        <div className="flex-1 md:px-[40px]  lg:px-[60px] mx-auto">
          ...
          <div className="block h-full w-full">
            {/* <p>{produto[0].name}</p> */}
            <div style={{ position: 'sticky', top: '0px', height: '45px', zIndex: 999, flex: 1, }} className=" bg-zinc-100 py-2 my-4 h-9 md:hidden">
            ...
            <div className="w-full relative flex flex-wrap rounded-xl bg-white py-2">
              {modeView == 'grid' ? produto.map((e) => (
                <ProductHorizCategory e={e} />
              )) : produto.map((e) => (
                <ProductVertCategory e={e} />
              ))}
            </div>
          </div>
        </div>
