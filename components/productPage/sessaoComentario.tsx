      ...
      >
            ...
            <div className="flex items-center flex-col lg:flex-row justify-between">
              ...
              <div className="w-full lg:hidden">
                ...
                <button className="w-full lg:w-[200px] py-2  transition-colors flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 rounded-md text-lg text-black font-medium">
                  <p className="text-md text-white">{t('productPage.rateThisProduct')}</p>
                </button>
              </div>
              <button className="bg-blue-500 hover:bg-blue-600 w-full hidden lg:flex lg:w-[200px] py-2 transition-colors  items-center justify-center gap-2 rounded-md text-lg text-white font-medium">
                <p className="text-md">{t('productPage.rateThisProduct')}</p>
              </button>
            </div>
          ...
          <div className="rounded mb-8 self-start bg-white flex gap-2 py-5 px-5">
            <div className="-lg p-2 flex flex-col gap-3 ">
              ...
              <div className="py-2 lg:px-0 bg-white rounded-lg">
                <p>
                  {produto.feedback.length + 1} {t('productPage.ratedThisProduct')}
                </p>
              </div>
            </div>
          </div>
        ...
        py-5 px-5 w-full bg-white`}
                >
                  <div className="flex items-center gap-2">
                    <ImgComments
                      ...
                      width={46}
                      rounded="50%"
                      img={processLink(comment.buyer.buyerImage)}
                    />
                    <div className="flex flex-col">
                      <p className="font-semibold">
                        {comment.buyer.buyerTitle}{' '}
                        <span className="font-normal">
                          {t('productPage.acquiredThe')}{' '}
                        </span>{' '}
                      </p>
                      <span className="text-[13px] font-medium">
                        {' '}
                        {comment.review.itemSpecInfo}
                      </span>
                    </div>
                  </div>
                  <div className="mb-3">
                  ...
                  <ul className="flex gap-2">
                    {comment.review.reviewImages !== null &&
                      comment.review.reviewImages.map((src, index) => (
                        <li key={index}>
                          <ImgComments
                            height={64}
                            width={64}
                            rounded="0px"
                            img={src !== null && processLink(src)}
                          />
                        </li>
                      ))}
                  </ul>
          ...
          <SlArrowRight
            ...
            className={`absolute ${
              commentsLength >= produto.feedback.length ? 'hidden' : 'inline'
